...
3. 切换到 **存储配置** 标签页，填写阿里云 OSS 信息并保存。
...
在 **存储配置** 标签页填写和管理 OSS 连接信息：

| 字段 | 说明 |
|------|------|
| 配置名称 | 显示用名称，多配置时便于区分 |
| 上传方式 | 目前仅阿里云 OSS 可用 |
| AccessKey ID | 用于签名 OSS 上传请求 |
...
2. **上传并替换** — 单张图片逐个手动上传。
3. **再次上传覆盖** — 对已管理的远程图片，选择本地文件重新上传覆盖 OSS 对象。

上传完成后，成功替换的链接变为 OSS 远程 URL，失败的保持本地链接不变。页面上方会显示汇总结果。
